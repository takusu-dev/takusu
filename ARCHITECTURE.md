...
Rust のワークスペースとして14のクレートで構成され、スケジューリングコア・REST APIサーバー・
...
```
   ...
   └──────────┘    └──────────────┘
   ┌──────────┐    ┌──────────────┐    ┌──────────────┐
   │takusu-   │    │ takusu-      │    │ funasr_server│
   │storage   │    │ worker       │    │ (Python STT) │
   │(Storage  │    │ (Cloudflare  │    └──────────────┘
   │ trait)   │    │  Worker/CDYLIB)│
   └──────────┘    └──────────────┘
```
...
### 3.1 takusu-serve (単一サーバー)

`axum` + `sqlx` (SQLite) の REST API サーバー。

#### リクエストフロー

```
HTTP Request → Router → Auth Middleware → Handler → DB / Planner → Response
                                                              │
                                                              ▼
...
| `/api/tasks/:id` | GET/PATCH/PUT/DELETE | タスクCRUD |
| `/api/tasks/import/ical` | POST | iCalインポート |
| `/api/habits` | GET/POST | 習慣一覧・作成 |
| `/api/habits/:id` | GET/PATCH/PUT/DELETE | 習慣CRUD |
| `/api/schedule` | GET | 現在のスケジュール取得 |
| `/api/schedule/generate` | POST | スケジュール生成 |
| `/api/schedule/reschedule` | POST | 部分再スケジュール |
| `/api/schedule/entries/:id` | PATCH | エントリ移動 |
| `/api/schedule` | DELETE | スケジュールクリア |
| `/api/settings` | GET/PATCH | ユーザー設定 |
| `/api/tokens` | GET/POST | トークン管理 |
| `/api/tokens/:id` | DELETE | トークン失効 |
| `/api/sync/settings` | GET/PUT | Google Calendar連携設定 |
| `/api/sync/oauth-url` | GET | OAuth2 URL生成 |
| `/api/sync/oauth-callback` | POST | OAuth2 コールバック |
| `/api/sync/trigger` | POST | 手動同期トリガー |
...
1. ルートトークン（TAKUSU_ROOT_TOKEN）と直接比較
2. 不一致なら SHA-256 ハッシュ → tokens テーブル検索
3. revoked_at IS NULL のものを有効とみなす
4. トークンが見つからない → 401 Unauthorized
```

#### DB スキーマ (SQLite)

```
tokens:      id (UUIDv7), name, hashed_token, created_at, revoked_at
tasks:       id (UUIDv7), title, description, status, start, end, cost_avg, cost_sigma,
             depends (JSON), parallelizable, allows_parallel, abandonability,
             ical_uid (unique), habit_id (FK), created_at, updated_at
habits:      id (UUIDv7), title, description, rule (JSON), start, end, duration,
             parallelizable, allows_parallel, abandonability, created_at, updated_at
schedules:   id='active', entries (JSON), created_at, updated_at
settings:    id='default', tz, sleep_start, sleep_end
google_cal_settings: id='default', client_id, client_secret, refresh_token, calendar_id
google_cal_events:   task_id (PK, FK → tasks CASCADE), event_id
```

### 3.2 takusu-local (プラグイン可能ストレージ)

`takusu-serve` と同一のAPIサーフェスを持つが、`Storage` トレイト経由で
ストレージバックエンドを切り替えられる。
...
### 6.2 STT (takusu-audio/src/funasr.rs + funasr_server/)

```
[Client]                         [FunASR Server (Python)]
   │ WebSocket connect                │
   │──── {"type":"start",...} ────────▶│
...
- Irodori-TTS: OpenAI 互換 `POST /v1/audio/speech`
- 参照音声: `./refs/` ディレクトリから最初の音声ファイルを自動選択
- フォーマット: WAV, 速度調整可
...
| 統合 | takusu-serve | axum oneshot + インメモリSQLite | 24 |
| 統合 | takusu-local | 同上 | ~24 |
...
### 12.3 なぜ2つのサーバー実装があるのか？

- `takusu-serve`: シンプルな単一サーバー。SQLite 直アクセス。
- `takusu-local` + `takusu-worker`: Cloudflare Workers への移行パス。
  `Storage` トレイトで抽象化することで、ハンドラコードを再利用。
- 現状は両者が並存している。将来的には `takusu-local` に統合される可能性がある。
...
| 4 | `crates/takusu-serve/src/handler/schedule.rs` | サーバーでの使い方 |
| 5 | `crates/takusu-serve/src/model.rs` | APIのデータモデル |
...
| `TaskRow`, `CreateTask`, `UpdateTask` | `takusu-serve/src/model.rs` |
| `ScheduleRow`, `ScheduleEntry` | `takusu-serve/src/model.rs` |
| `AppError` | `takusu-serve/src/error.rs` |
...
| サーバー統合テスト | `takusu-serve/tests/integration.rs` (1024行) |
