        ...
        >
          ...
          {shortcuts && shortcuts.length > 0 && (
            ...
            >
              {shortcuts.map((sc, i) => {
                ...
                // Skip shortcuts that would violate the minimumDate
                // constraint (e.g. a "1時間後" chip when the start date is
                // already more than an hour ahead).
                const disabled =
                  minimumDate !== undefined && candidate < minimumDate;
                return (
                  <Pressable
                    key={`${sc.label}-${i}`}
                    disabled={disabled}
                  ...
                  >
                    <Text
                      style={[
                        styles.shortcutText,
                        {
                          color: disabled ? colors.grayLight : BRAND_COLOR,
                        },
                      ]}
                    >
                      {sc.label}
          ...
          {mode === 'time' ? (
            <Pressable
              style={[
                styles.fieldRow,
                { backgroundColor: dark ? '#2A2438' : '#F8F5FC' },
              ]}
